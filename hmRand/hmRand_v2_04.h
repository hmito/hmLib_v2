...
public:
	hmRand(){
		...
		init_gen_rand(static_cast<uint>(std::time(NULL)));
		RandNormActive=false;
		RandNorm=0.;
	}
	...
	unsigned int operator()(unsigned int _under){return gen_rand32()%_under;}
	...
	double gen_randNorm(void){
		if(RandNormActive){
			...
			RandNormActive=true;
			double tmp1,tmp2;
			tmp1=1.-genrand_real2();
			tmp2=1.-genrand_real2();
			RandNorm=(sqrt(log(tmp1)*-2.)*cos(2*pi()*tmp2));
			return (sqrt(log(tmp1)*-2.)*sin(2*pi()*tmp2));
		}
	}
};
namespace hm{
	...
	double randNorm(double ave=0.,double sd=1.);
	hmRand& getGlobalRand();
	...
	inline double randNorm(hmRand& rand,double ave=0.,double sd=1.){
		return rand.gen_randNorm()*sd+ave;
	}
}
#endif
