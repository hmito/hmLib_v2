...
int WinRS::init(int addr, int bps, char* mode)
{

    char paraBuff[256];

    std::sprintf(paraBuff, "\\\\.\\COM%d", addr);
    ...
    std::sprintf (paraBuff, "baud=%d parity=%c data=%c stop=%c", bps, mode[1], mode[0], mode[2]);
...
ifLine::stat WinRS::talk(char *pMess)
{
  unsigned long dLen;
  char *mess;
...
ifLine::stat WinRS::putN(char *mess, int len)
{
