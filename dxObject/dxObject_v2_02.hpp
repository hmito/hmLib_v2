...
public:
	dxFn(){pDrawFunc=0;}
	dxFn(I_pFunc_rDXO _pDrawFunc){pDrawFunc=_pDrawFunc;}
	dxFn(const dxFn& _Fn){pDrawFunc=_Fn.pDrawFunc;}
	void set(I_pFunc_rDXO _pDrawFunc){pDrawFunc=_pDrawFunc;}
	int operator()(dxO& dxo){
		if(pDrawFunc==0)return 0;
		else return pDrawFunc(dxo);
	}
};
template<class DrawFuncType>
class dxFnObject_:public dxSubObject{
public:
	DrawFuncType* pDrawFunc;
private:
	void local_set(DrawFuncType& _DrawFunc){
		pDrawFunc=&_DrawFunc;
	}
	void local_copy(const dxFnObject_<DrawFuncType>& _dxo){
		pDrawFunc=_dxo.pDrawFunc;
	}
protected:
	void copy(const dxFnObject_<DrawFuncType>& _dxo){
		local_copy(_dxo);
	}
public:
	dxFnObject_(){local_set(0);}
	dxFnObject_(DrawFuncType& _DrawFunc){local_set(_DrawFunc);}
	dxFnObject_(const dxFnObject_<DrawFuncType>& _dxo){local_copy(_dxo);}
	dxFnObject_& operator=(const dxFnObject_<DrawFuncType>& _dxo){
		if(this!=&_dxo)copy(_dxo);
		return *this;
	}
	void set(DrawFuncType& _DrawFunc){local_set(_DrawFunc);}
	virtual int draw(Pint _pos,bool _active=true,dxDMode _mode=dxDMode()){
		dxO dxo(_pos,_active,_mode);
		int ans=(*pDrawFunc)(dxo);
		area=dxo.getArea();
		return ans;
	}
};
typedef dxFnObject_<dxFn> dxFnObject;
